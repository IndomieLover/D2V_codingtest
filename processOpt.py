...
class imageProcess:
    ...
    def processImage(self):
        #increase contrast
        grayFrameCalc = self.rawFrame[self.width/2-10:self.width/2+10,self.height/2-10:self.height/2+10]
        croppedImg = self.rawFrame[self.width/2-40:self.width/2+40,self.height/2-40:self.height/2+40]

        lab= cv2.cvtColor(grayFrameCalc, cv2.COLOR_BGR2LAB)
        l, a, b = cv2.split(lab)
        clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8,8))
        cl = clahe.apply(l)
        limg = cv2.merge((cl,a,b))
        grayFrame = cv2.cvtColor(limg, cv2.COLOR_LAB2BGR)
        grayFrame = cv2.cvtColor(grayFrame, cv2.COLOR_BGR2GRAY)

        #crop image to the center
        self.meanvalue2=cv2.mean(grayFrameCalc)
        grayFrame = cv2.GaussianBlur(croppedImg,(3,3), 1)

        #search for edges
        self.meanvalue=cv2.mean(grayFrameCalc)
        print(self.meanvalue2)
            ...
            for j in range(i+1,len(self.gradient)):
                if(abs(float(self.gradient[i]-self.gradient[j]))<0.30):
                    ...
                    if ((abs(x1-_x1)>15 and abs(x2-_x2)>15) or (abs(y1-_y1)>15 and abs(y2-_y2)>15)) and abs(length1-length2)<10 and length3 > 20 and length1>10 and length2 >10:
                        ...
                        if _x2>x1:
                            cv2.arrowedLine(self.rawFrame,(self.width/2-40+x1,self.height/2-40+y1),(self.width/2-40+_x2,self.height/2-40+_y2),(0,0,255),2)
                        else:
                            cv2.arrowedLine(self.rawFrame,(self.width/2-40+_x2,self.height/2-40+_y2),(self.width/2-40+x1,self.height/2-40+y1),(0,0,255),2)
                        return
